...
class Library_people(db.Model): 
    ...
    added_Year = db.Column(db.String(5))
    people_username = db.relationship('User' , backref = 'people')
    people_penalty = db.relationship('penalty' , backref = 'people_penalty')
...
class book(db.Model):
  ...
  book_shelf = db.relationship('book_shelf', backref= 'book_shel' ,cascade="all, delete-orphan" )
...
class book_loan(db.Model): 
    ...
    penalty_id = db.relationship('penalty' , backref = 'penalty_loan')
...
class penalty (db.Model):
    ...
    id_loan = db.Column(db.Integer(), db.ForeignKey('book_loan.req_id'))
...
with app.app_context():
   ...
   ##test = author.query.filter_by(name='bader').first()
    #search_book = book_item.query.filter_by(barcode='98884943').first()

    #test = book_loan.query.filter_by(book_barcode = "2").first()

    #print(test.actual_return)

    #if search_book.isbn_code == None :
      #  print ("HI")
   # print (search_book.isbn_code)
